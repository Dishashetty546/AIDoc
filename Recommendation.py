...
le = LabelEncoder()
le.fit(y)
y_encoded = le.transform(y)  # renamed for clarity
# print(y_encoded)  # Convert string labels to integers
...
# Save the trained SVC model (ensure 'models' directory exists)
...
def get_predicted_value(patient_symptoms):
    ...
    predicted_disease = svc.predict([input_vector])[0]  # corrected here
...
print("Processed symptoms list:", user_symptoms)

predicted_disease = get_predicted_value(user_symptoms)
print("Predicted Disease:", diseases_list[predicted_disease])

#fetch data accordingly from database
def helper(dis):
...
desc, pre, med, die, wrkout = helper(diseases_list[predicted_disease])
